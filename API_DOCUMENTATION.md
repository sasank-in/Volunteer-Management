...
### Get My Organized Events (Organizer/Admin)
```http
...
### Cancel Participation (Volunteer only)
```http
...
### Update Volunteer Role (Organizer/Admin)
```http
PUT /api/participations/events/{eventId}/volunteers/{volunteerId}/role?role=Team Leader
Authorization: Bearer <token>
```

### Mark Attendance (By Participation ID, Organizer/Admin)
```http
...
### Submit Feedback (Volunteer only)
```http
